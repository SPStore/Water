...
typedef NS_ENUM(NSInteger, SPLoadingHUDAppearance) {
    ...
    SPLoadingHUDAppearanceCircle,  // 圆，如果是矩形，则两边为半圆，中间为矩形
...
// 注：如果以类方法显示和隐藏，内部设置了removeFromSuperViewOnHide，隐藏HUD会自动从父View中移除，外界无法修改removeFromSuperViewOnHide; 如果以对象方法显示和隐藏,默认会在隐藏后从父view中移除，可以设置不移除
